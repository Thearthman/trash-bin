...
maxId = []
maxData = []
sortedList = pd.DataFrame({"Alice": [], "bob": []})
avgSortedList = pd.DataFrame({'0': [], '1': []})
mask = pd.DataFrame({'Alice': [], 'Bob': []})
rebounce = False
iterate = [0]
newRawData = pd.DataFrame({"Alice": rawData.iloc[:,0], "Bob":rawData.iloc[:,1]})
idxNumberdData = newRawData.rename(columns={"Alice": "0", "Bob": "1"})
...
adList = [mp.getad(replacementList, candidateNumber=2)]
...
for i in range(30):
    ...
    iterate.append(i+1)
    adList.append(mp.getad(replacementList, candidateNumber=2))


mp.printdata(sortedList,avgSortedList,adList)
mp.drawplot(iterate,adList,"A vs B Absolute Deviation against iteration")
