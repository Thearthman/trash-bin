...
def Update_Value_Matrix(item_distributed: int = 0, who_to_give: int = 0,
                        Value_Matrix: list = []):  # item_distributed is the location or index of the itgem in the order list
    ...
    for changing_position in Range_to_be_changed:  ##### If you wanna go from the beginning, it is really great
        ...
        R = (100 + Correlation_Coefficient * Gain_Matrix[item_order][changing_item_order]) / 100 * \
            V_Current[changing_position][who_to_give]  # Both the variables are item number not location
        Value_Matrix[changing_position][who_to_give] = R
        for else_people in who_doesnot_get:
            R = (100 + Correlation_Coefficient * Loss_Matrix[item_order][changing_item_order]) / 100 * \
                V_Current[changing_position][else_people]  # Both the variables are item number not location n
...
while current_row_number < num_of_rows:
    ...
    if whether_repeat_values:
        ...
        who_to_give = J.index(
            min(J))  # Anchor point. Via changing this, the optimization instance variable can be changed.
...
sortedList = pd.DataFrame(Value_Distribution_Matrix)
...
with pd.ExcelWriter("NewDataOut_GPL.xlsx", mode="a", if_sheet_exists="replace") as writer:
    printList.to_excel(writer, sheet_name="AB+C")
