...
# ------------------------------------------------------------------------------#
newSortList = pd.DataFrame({"Alice": sortedList.iloc[:, 0], "Bob": sortedList.iloc[:, 1],"Charlie": sortedList.iloc[:, 2]})
newAvgList = pd.DataFrame({"Alice": avgSortedList.iloc[:, 0], "Bob": avgSortedList.iloc[:, 1],"Charlie": avgSortedList.iloc[:, 2]})
newRawData = pd.DataFrame({"0": rawData.iloc[:, 0], "1": rawData.iloc[:, 1],'2': rawData.iloc[:,2]})
newCandidateNumber = len(newRawData.columns)
newMax = []
newMaxId = []
mask = []
for j in range(30):
    mask.append(j) if sortedList.iloc[j, 0].item() != 0 or sortedList.iloc[j, 1].item() != 0 or sortedList.iloc[j, 2].item() != 0 else None
print(mask)
...
with pd.ExcelWriter("DataOut.xlsx", mode="a", if_sheet_exists="replace") as writer:
    printList.to_excel(writer, sheet_name="ABC+D+E")
