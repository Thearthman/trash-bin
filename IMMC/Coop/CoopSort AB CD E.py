...
# Starts Iterating for C D
# ------------------------------------------------------------------------------#
newSortList = pd.DataFrame({"Charlie": sortedList.iloc[:, 2], "David": sortedList.iloc[:, 3]})
newAvgList = pd.DataFrame({"Charlie": avgSortedList.iloc[:, 2], "David": avgSortedList.iloc[:, 3]})
newRawData = pd.DataFrame({"0": rawData.iloc[:, 2], "1": rawData.iloc[:, 3]})
newCandidateNumber = len(newRawData.columns)
newMax = []
newMaxId = []
mask = []
for j in range(30):
    mask.append(j) if sortedList.iloc[j, 2].item() != 0 or sortedList.iloc[j, 3].item() != 0 else None
print(mask)
...
with pd.ExcelWriter("DataOut.xlsx", mode="a", if_sheet_exists="replace") as writer:
    printList.to_excel(writer, sheet_name="AB+CD+E")
